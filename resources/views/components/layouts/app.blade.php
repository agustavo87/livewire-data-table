<x-layouts.base>
    <section x-data="{ sideBar: false }" class="min-h-screen bg-gray-50">
        <nav @click.away="sideBar = false" :class="{ '-translate-x-full' : !sideBar, 'translate-x-0' : sideBar }" class="fixed top-0 left-0 z-20 h-full flex flex-col justify-between transition origin-left transform bg-gray-900 w-60 md:translate-x-0">
            <div class="absolute top-0 right-0 -mr-14 p-1">
                <button
                x-show="sideBar"
                @click="sideBar = false" class="flex items-center justify-center h-12 w-12 rounded-full focus:outline-none focus:bg-gray-600" aria-label="Close sidebar" style="display: none;">
            ...
            <div>
                ...
                <nav class="text-sm font-medium text-gray-500 overflow-x-hidden overflow-y-auto" aria-label="Main Navigation">
                    ...
                    <a href="{{ route('logout') }}"class="flex items-center px-4 py-3 transition cursor-pointer group hover:bg-gray-800 hover:text-gray-200">
                        <x-icons.logout class="flex-shrink-0 w-5 h-5 mr-2 text-gray-400 transition group-hover:text-gray-300" />
                        <span>Logout</span>
                    </a>
                </nav>
            </div>
        ...
        <div class="ml-0 transition md:ml-60">
            <div class="md:hidden pl-1 pt-1 sm:pl-3 sm:pt-3">
                <button @click.stop="sideBar = true" class="-ml-0.5 -mt-0.5 h-12 w-12 inline-flex items-center justify-center rounded-md text-gray-500 hover:text-gray-900 focus:outline-none focus:bg-gray-200 transition ease-in-out duration-150" aria-label="Open sidebar">
